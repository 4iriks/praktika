# Worker PyAnswer — архитектура подэтапа 5.1
...
На 5.1 worker обрабатывает `SOURCE_SYNC` в ограниченном fetch/checkpoint режиме. Полное
сохранение и обработка документов появятся в 5.2. `DOCUMENT_REINDEX` и `FULL_REINDEX` не
выдаются за выполненную поисковую индексацию.
...
Checkpoint хранит режим, фиксированный `todate`, текущую страницу, счётчики запросов и последнюю
успешную безопасную единицу работы. Он обновляется только после успешного commit соответствующего
batch. Поэтому restart worker или Docker не приводит к пропуску уже загруженной страницы.
...
Полная загрузка 25 000 веток на подэтапе 5.1 автоматически не запускается.
